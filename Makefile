...
test-backend-integration: install
	...
	$(RUN) coverage.space jacebrowning/template-flask-demo integration
...
test-backend-all: install
	...
	$(RUN) coverage.space jacebrowning/template-flask-demo overall
...
run: install ## Run the applicaiton
	FLASK_ENV=dev $(RUN) python manage.py runserver
